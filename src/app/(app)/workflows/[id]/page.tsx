...
export default function WorkflowPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (workflowQuery.isLoading) {
    return (
      <div className="flex h-full min-h-[calc(100vh-4rem)] items-center justify-center bg-background">
        <div className="text-muted-foreground animate-pulse">Loading workflow canvas...</div>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-full min-h-[calc(100vh-4rem)] items-center justify-center bg-background">
        <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 p-6 text-center text-rose-300">
          <p className="font-semibold text-lg mb-2">Error loading workflow</p>
          <p className="text-sm">Please ensure the backend is running and the workflow exists.</p>
        </div>
      </div>
    );
  }
