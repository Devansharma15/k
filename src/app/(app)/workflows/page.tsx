...
export default function WorkflowsIndexPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (e: Error) => {
      setErrorMsg(`Failed to create: ${e.message}`);
    }
  });
          ...
          <div className="flex items-start justify-between gap-6">
            ...
            <div className="text-right">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Workflow className="h-4 w-4" />
                {createMutation.isPending ? "Creating..." : "New Workflow"}
              </button>
              {errorMsg ? <p className="mt-2 text-xs text-rose-500">{errorMsg}</p> : null}
            </div>
          </div>
